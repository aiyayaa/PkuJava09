...
/*算法核心类，算法的主体思想分三个步骤，
 *第一步：根据双方的当前的形势循环地假设性的分别给自己和对方下一子（在某个范围内下子），
 *		并判断此棋子能带来的形势上的变化，如能不能冲4，能不能形成我方或敌方双3等，
...
//			}
			...
			//在“纵向”这个方向上重复上面的步骤
			firstAnalysisResult = tryAndCountResult(comuters,humans, computerPoint, ZONG);
			...
			//“纵向”
			firstAnalysisResult = tryAndCountResult(humans,comuters, computerPoint, ZONG);
			computerPoint.setX(x).setY(y);
			if(firstAnalysisResult!=null){//死棋，不下
				if(firstAnalysisResult.count==5)
					humanPoint = computerPoint;
